//
//  MovieCreditsTableViewController.swift
//  cinemaDB
//
...
class MovieCreditsTableViewController: UITableViewController, APIControllerProtocol {
    ...
    override func viewDidLoad() {
        ...
        let urlPath = "\(Config.apiMovie)\(id)/credits?api_key=\(Config.apiKey)"
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "MovieCreditCell")! as! MovieCreditCell
    ...
    func didReceiveAPIResults(_ results: AnyObject, URL: String, key: String) {
        if URL.range(of: Config.apiImageURL) != nil {
            ...
            DispatchQueue.main.async(execute: {
                ...
                if let cellToUpdate = self.creditsTableView.cellForRow(at: indexPath!) as! MovieCreditCell? {
            ...
            DispatchQueue.main.async(execute: {
                self.credits = MovieCredit.movieCreditsWithJSON(results as! NSArray)
                self.creditsTableView!.reloadData()
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "MovieToPersonSegue" {
